...
if [ -f "/workspaces/circleci-mcp/.devcontainer/.zshrc" ]; then
    echo "INFO: Copying .zshrc to $USER_HOME_DIR/.zshrc"
    cp "/workspaces/circleci-mcp/.devcontainer/.zshrc" "$USER_HOME_DIR/.zshrc"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.zshrc"
else
    echo "INFO: /workspaces/circleci-mcp/.devcontainer/.zshrc not found, skipping copy."
fi

if [ -f "/workspaces/circleci-mcp/.devcontainer/.p10k.zsh" ]; then
    echo "INFO: Copying .p10k.zsh to $USER_HOME_DIR/.p10k.zsh"
    cp "/workspaces/circleci-mcp/.devcontainer/.p10k.zsh" "$USER_HOME_DIR/.p10k.zsh"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.p10k.zsh"
else
    echo "INFO: /workspaces/circleci-mcp/.devcontainer/.p10k.zsh not found, skipping copy."
fi

if [ -f "/workspaces/circleci-mcp/.devcontainer/.tmux.conf" ]; then
    echo "INFO: Copying .tmux.conf to $USER_HOME_DIR/.tmux.conf"
    cp "/workspaces/circleci-mcp/.devcontainer/.tmux.conf" "$USER_HOME_DIR/.tmux.conf"
    sudo chown "$CURRENT_USER:$CURRENT_USER" "$USER_HOME_DIR/.tmux.conf"
else
    echo "INFO: /workspaces/circleci-mcp/.devcontainer/.tmux.conf not found, skipping copy."
fi
...
cd "/workspaces/circleci-mcp" 2>/dev/null || true
...
if [ -d "/workspaces/circleci-mcp/.venv/bin" ]; then
    export PATH="/workspaces/circleci-mcp/.venv/bin:$PATH"
fi
EOF
...
git config --global --add safe.directory /workspaces/circleci-mcp
...
echo "    cd /workspaces/circleci-mcp && bash scripts/cloud_login.sh"
