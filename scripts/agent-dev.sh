...
# "circleci-mcp-dev" (the repo name plus the "-dev"
...
# than adding a second one - a stale "circleci-mcp-dev" is overwritten by
...
PROJECT_NAME="circleci-mcp"
AGENT_NAME="circleci-mcp-dev"
WORKSPACE="/workspaces/circleci-mcp"
...
if [ -x "/workspaces/circleci-mcp/scripts/agent-dev.sh" ]; then
  "/workspaces/circleci-mcp/scripts/agent-dev.sh" start >/dev/null 2>&1 || true
fi
POST_START_HOOK
}
